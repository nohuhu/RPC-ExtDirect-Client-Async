...
AnyEvent::HTTP tries to reuse existing connection for idempotent requests,
but for some obscure reason this does not always work as expected. As the
consequence, first GET request (Ext.Direct API declaration retrieval)
succeeds, while subsequent GET requests, e.g. event polling, will fail with
'Connection reset by peer' error. Which is not very helpful, to say the
least.

To work around this issue, persistent connections has been disabled by
default for all types of requests, including idempotent. If you are sure
that this issue will not be a problem for you and want to use persistent
connections, pass C<persistent => 1> parameter either to Client constructor,
or to the individual C<poll_async()> calls.
